"""Shared pieces of the on-device continual-triage demo (drafts/blog-online-sdft-triage.md).
...
the LFM2.5-230M model helpers used by both entry points:
  ...
  scripts/run_baselines.py   ZS / ICL / RAG arms (+ k sweep) -> outputs/triage-showcase/baselines.json
  scripts/run_sdft.py        the online SDFT loop -> results.json + the blog figure
...
def gen_item(category: str, rng: random.Random) -> dict:
    """One synthetic inbox item: {channel, sender, subject, snippet, category}."""
